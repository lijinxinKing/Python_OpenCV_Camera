...
class Object_detect():
    ...
    def run(self):
        getDeviceName()
        self.ua = ultraArm(deviceName, 115200)
        self.ua.go_zero()
...
if __name__ == "__main__":
    ...
    while cv2.waitKey(100) < 0:
       # read camera
        ...
        # calculate params of the coords between cube and ultraArm P340 
        # 计算立方体和 mycobot 之间坐标的参数
        if nparams < 10:
            if detect.get_calculate_params(frame) is None:
                cv2.imshow("figure", frame)
                continue
            ...
            nparams += 1
            # calculate and set params of calculating 
            # real coord between cube and ultraArm P340
