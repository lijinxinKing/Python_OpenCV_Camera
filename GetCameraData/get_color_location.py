...
def getColor():
        ...
        piex = (MaxW + MaxH) / 2
        piex_distance = round(piex / 15,2)
        result = (MaxX,MaxY,MaxW,MaxH,piex_distance)
        print(result)
        return result
        global num
        num = num + 1
...
def GetRealTime():
    ...
    while True:
        ...
        #cv2.circle(frame, point, point_size, point_color, thickness)
        #cv2.putText(frame, "Center: " + str(x) +", "+ str(y)+","+str(getCurrentHigh), (x, y - 5), font, 0.3, point_color, 1)
        hsv_img = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
        #result = color_detect(hsv_img)
        #(0, 537.0, 179.5)
        #(1, 537.0, 180.0)
        #print(result)
        mask_blue = cv2.inRange(hsv_img, lower_blue, upper_blue)
        ...
        for cnt3 in contours3:
            ...
            piex = (w3 + h3) / 2
            pid = round(piex / 15,2)
            cv2.rectangle(frame, (x3, y3), (x3 + w3, y3 + h3), (153, 153, 0), 1)
            setStr = str(x3)+","+str(y3)+","+str(pid)+","+str(w3)+","+str(h3)
            print(setStr)
            cv2.putText(frame,setStr,(x3, y3 - 5),font,0.3,(153, 153, 0), 1)
        cv2.imshow("dection", frame)
